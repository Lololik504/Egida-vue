<template>
  <div class="container">
    <Loader v-if="loading"/>
    <div v-else>
      <h5 class="center">Межэтажные перекрытия</h5>
      <form @submit.prevent="save">
        <div class="q-pa-md">
          ...
          <br/>
          <div v-if="!data.inter_floor_overlapping_act" class="input-roof-photo">
            <label>Акт обследования технического состояния (экспертной оценки специализированной организации)</label>
            <q-file
                v-model="data.inter_floor_overlapping_act"
                outlined
                :disable="disable"
          ...
          <div v-else style="margin-bottom: 25px">
            ...
            <div class="q-gutter-sm">
              <button v-if="!changedAct" class="btn blue" @click.prevent="showDoc(data.inter_floor_overlapping_act)">
                Просмотреть файл
              </button>
    ...
    async save() {
      try {
        ...
        for (let key in this.data) {
          if ((key === 'inter_floor_overlapping_act' && typeof this.data[key] === 'string')) {
            continue
          }
