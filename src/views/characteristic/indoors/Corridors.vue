      ...
      <form @submit.prevent="save">
        <div class="q-pa-md">
          <q-card flat bordered class="my-card">
            ...
            <div v-if="!data.corridors_act" class="input-roof-photo" style="margin-left: 25px">
              <label>Акт обследования технического состояния (экспертной оценки специализированной организации)</label>
              <q-file
                  v-model="data.corridors_act"
                  outlined
                  :disable="disable"
            ...
            <div v-else style="margin-bottom: 25px; margin-left: 25px">
              ...
              <div class="q-gutter-sm">
                <button v-if="!changedAct" class="btn blue"
                        @click.prevent="showDoc(data.corridors_act)">
                  Просмотреть файл
                </button>
                <button class="btn blue"
                        @click.prevent="data.corridors_act = null;">
                  Изменить файл
                </button>
                <button v-if="getPermission" class="btn blue"
                        @click.prevent="deleteDoc('corridors_act')">
                  Удалить файл
                </button>
              </div>
            </div>
    ...
    async save() {
      try {
        ...
        for (let key in this.data) {
          if (key === 'corridors_act' && typeof this.data[key] === 'string') {
            continue
          }
          (key === 'corridors_ok_percent' && (this.data[key] === '' || this.data[key] == null)) ? this.data[key] = 0 : null;
          (key === 'corridors_warning_percent' && (this.data[key] === '' || this.data[key] == null)) ? this.data[key] = 0 : null;
          (key === 'corridors_emergency_percent' && (this.data[key] === '' || this.data[key] == null)) ? this.data[key] = 0 : null;
